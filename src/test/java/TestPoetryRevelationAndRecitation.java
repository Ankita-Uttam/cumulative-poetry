...
public class TestPoetryRevelationAndRecitation {
    ...
    private RevealByDay reveal = new RevealByDay();
    ...
    public void testReciteStory() {
        ...
        Assert.assertNotNull("Recitation is not null", recite.recite(getStory()));
        Assert.assertTrue(recite.recite(getStory()).startsWith("Day 1 -"));
        Assert.assertTrue(recite.recite(getStory()).split("\n\n").length == Constants.LAST_DAY);
    }
}
